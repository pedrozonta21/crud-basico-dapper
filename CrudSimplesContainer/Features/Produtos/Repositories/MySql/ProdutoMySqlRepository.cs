...
namespace CrudSimplesContainer.Features.Produtos.Repositories.MySql;

public class ProdutoMySqlRepository : IProdutoRepository
{
    private readonly IFabricaDeConexao _fabricaDeConexao;

    public ProdutoMySqlRepository(IFabricaDeConexao fabricaDeConexao) => _fabricaDeConexao = fabricaDeConexao;
    ...
    public async Task<int> CadastrarProduto(ProdutoModel produtoModel)
    {
        ...
        var codigoProduto = await dbTransaction.Connection.ExecuteScalarAsync<int>(
            @"INSERT INTO produtos (Nome, ValorUn, Qtde) VALUES (@Nome, @ValorUn, @Qtde);SELECT LAST_INSERT_ID();", 
            produtoModel, 
            dbTransaction);
